...
static int readOperand(const vector<uint8_t> &code, int &ip) {
    int val = static_cast<int>(code[ip])
            | (static_cast<int>(code[ip + 1]) << 8)
            | (static_cast<int>(code[ip + 2]) << 16)
            | (static_cast<int>(code[ip + 3]) << 24);
    ip += 4;
    return val;
}
...
void VM::execute(Opcode op) {
    ...
    case Opcode::SUB: {
        ...
        expectInteger(a, "SUB", ip); expectInteger(b, "SUB", ip);
        push(Value(a.intVal - b.intVal));
        break;
    }
    ...
    case Opcode::MUL: {
        Value b = pop(); Value a = pop();
        expectInteger(a, "MUL", ip); expectInteger(b, "MUL", ip);
        push(Value(a.intVal * b.intVal));
        break;
    }
    ...
    case Opcode::DIV: {
        Value b = pop(); Value a = pop();
        expectInteger(a, "DIV", ip); expectInteger(b, "DIV", ip);
        if (b.intVal == 0) throw VMError("运行时错误：除数为零");
        push(Value(a.intVal / b.intVal));
        break;
    }
    ...
    case Opcode::MOD: {
        Value b = pop(); Value a = pop();
        expectInteger(a, "MOD", ip); expectInteger(b, "MOD", ip);
        if (b.intVal == 0) throw VMError("运行时错误：除数为零");
        push(Value(a.intVal % b.intVal));
        break;
    }
    ...
    case Opcode::LT: {
        Value b = pop(); Value a = pop();
        expectInteger(a, "LT", ip); expectInteger(b, "LT", ip);
        push(Value(a.intVal < b.intVal ? 1 : 0));
        break;
    }
    ...
    case Opcode::GT: {
        Value b = pop(); Value a = pop();
        expectInteger(a, "GT", ip); expectInteger(b, "GT", ip);
        push(Value(a.intVal > b.intVal ? 1 : 0));
        break;
    }
