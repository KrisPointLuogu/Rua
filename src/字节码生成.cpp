...
int BytecodeGenerator::visit(BinaryExpr &node)
{
	...
	if (node.op == TK_等号)
	{
		if (auto *id = dynamic_cast<Identifier *>(node.left.get()))
		{
			...
			if (slot < 0)
			{
				std::cerr << "内部错误：未找到变量 '" << id->name << "' 的寄存器" << std::endl;
				return 0;
			}
	...
	case TK_除号:
		...
		break;
	case TK_模:
		program.emit(Opcode::MOD, resultReg, leftReg, rightReg);
		break;
	case TK_等于:
		program.emit(Opcode::EQ, resultReg, leftReg, rightReg);
		break;
	case TK_不等于:
		program.emit(Opcode::NE, resultReg, leftReg, rightReg);
		break;
	case TK_大于:
		program.emit(Opcode::GT, resultReg, leftReg, rightReg);
		break;
	case TK_小于:
...
int BytecodeGenerator::visit(CallExpr &node)
{
	if (node.callee == "喵叫")
	{
		...
		for (size_t i = 0; i < node.args.size(); i++)
		{
			...
			if (i < node.args.size() - 1)
			{
				...
				program.emit(Opcode::MOVS, spaceReg, 0, 0, program.addString(" "));
	...
	if (funcIdx < 0)
	{
		std::cerr << "内部错误：未找到函数 '" << node.callee << "'" << std::endl;
		int resultReg = allocTemp();
		program.emit(Opcode::MOVI, resultReg, 0, 0, program.addConstant(0));
		return resultReg;
	}
