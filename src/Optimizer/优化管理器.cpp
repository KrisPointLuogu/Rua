...
void dumpTACFunction(const TACFunction& func) {
    std::cout << "  Function " << func.name << " (" << func.instructions.size() << " insts):\n";
    for (int ti = 0; ti < static_cast<int>(func.instructions.size()); ti++) {
        ...
        if (op == TACOpcode::MOVI) {
            ...
            std::cout << "JIF r" << j->cond.index << ", t=" << j->targetBlock << " f=" << j->fallBlock;
            ...
            std::cout << opname << " r" << b->rd.index << ", r" << b->rs1.index << ", r" << b->rs2.index;
        }
        std::cout << "\n";
    }
}
...
bool PassManager::runAll(TACProgram& program) {
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
        ...
        for (auto& pass : passes) {
            for (int i = 0; i < static_cast<int>(program.functions.size()); i++) {
                if (pass->run(program, i)) {
                    iterChanged = true;
#ifdef _DEBUG
                    std::cout << "  [" << pass->name() << "] func " << i << " changed:\n";
        ...
        changed = true;
        std::cout << "Optimization pass " << (iter + 1) << " completed, changed\n";
    }
    return changed;
}