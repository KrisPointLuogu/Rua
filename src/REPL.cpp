...
void 编译并运行(const string &源码)
{
    try
    {
        ...
        输出文本("完成，共 " + std::to_string(令牌列表.size()) + " 个 Token", "GG");

        输出文本("【阶段二】语法分析 ...", "青");
        Parser 解析器(令牌列表);
        auto 程序 = 解析器.parseProgram();
        输出文本("完成，共 " + std::to_string(程序->functions.size()) + " 个函数", "GG");
...
void 基本界面()
{
    ...
    if (全局内容.系统信息.find("Windows") != string::npos && 全局内容.系统信息.find("x86_64") != string::npos)
        输出文本("EXE位于 [" + 全局内容.系统信息 + "] 系统编译", "GG");
    else if (全局内容.系统信息.find("Windows") != string::npos && 全局内容.系统信息.find("x86") != string::npos)
        输出文本("EXE位于 [" + 全局内容.系统信息 + "] 系统编译，请注意，程序对此Windows版本可能存在兼容性问题", "YY");
    else if (全局内容.系统信息.find("Linux") != string::npos || 全局内容.系统信息.find("macOS") != string::npos)
        输出文本("EXE位于 [" + 全局内容.系统信息 + "] 系统编译，当前存在不稳定或功能异常风险", "YY");
    else
    {
