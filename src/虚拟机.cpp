...
void VM::run(const BytecodeProgram &prog)
{
	...
	static void *dispatch[] = {
		...
		&&op_mul, &&op_div, &&op_mod, &&op_eq, &&op_ne, &&op_lt, &&op_gt,
		&&op_jmp, &&op_jif, &&op_push, &&op_call, &&op_ret, &&op_print,
		&&op_le, &&op_ge,
	};

#define I32(a) ({ int _v; __builtin_memcpy(&_v, code.data() + (a), 4); _v; })
#define NEXT() goto *dispatch[code[ip]]

	ip = 0;
		NEXT();

	op_halt:     goto op_halt_end;
	op_movi:
	{
