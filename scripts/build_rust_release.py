...
if __name__ == "__main__":
    ...
    run(["cargo", "build", "--release"], cwd=str(NATIVE_DIR))
    src = NATIVE_DIR / "target" / "release" / BIN_NAME
    if not src.exists():
        print(f"[FAIL] 未找到编译产物: {src}", file=sys.stderr)
        sys.exit(1)
    shutil.copy2(str(src), str(BUILD_DIR / DEST_NAME))
    print(f"[OK] Rust 版 VM (Release) 编译完成，输出在 build/{DEST_NAME}")
