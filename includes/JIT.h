...
// 纯整数/算术函数走 JIT，含字符串/输出的函数退回到解释器。
//
...
private:
	...
	void emitModRMSIB(uint8_t mod, uint8_t oreg, uint8_t base, uint8_t index, uint8_t scale);
	...
	// 在当前位置写一个跳转，跳转到 label（后向跳转直接写；前向跳转记入 fixup 列表）
	void emitJumpToLabel(const char *cc, size_t label);
	void emitJumpToLabel(size_t label); // 无条件

private:
	std::vector<Label> labels;
	std::vector<std::pair<size_t, size_t>> forwardJumps; // (需要回填的codePos, labelIndex)
};
